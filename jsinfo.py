...
class JSINFO:
    ...
    def __init__(self):
        ...
        self.leak_info_patterns = {'Mail': r'(([a-zA-Z0-9][_|\.])*[a-zA-Z0-9]+@([a-zA-Z0-9][-|_|\.])*[a-zA-Z0-9]+\.((?!js|css|jpg|jpeg|png|ico)[a-zA-Z]{2,}))',
                                   'Swagger UI': r'((swagger-ui.html)|(\"swagger\":)|(Swagger UI)|(swaggerUi))',
                                   'Druid': r'((Druid Stat Index)|(druid monitor))',
                                   'URL Redirect': r'((redirect=)|(redirect_to=)|(redirect_url=)|(url=)|(jump=)|(jump_to=)|(target=)|(to=)|(link=)|(linkto=)|(domain=)|(callback=)|(fromurl=)|(goto=)|(oauth_callback=))',
...
`\___/'`\____)(_)(_) (_)(_)    (_____)
    ...
    async def FindLinkInPage(self, url):
        ...
        for path in paths:
            try:
                url_vul = url + path.strip()
                #logger.info(url_vul)
                resp_vul = await self.send_request(url_vul)
                if not resp_vul:
                    return None
                self.find_leak_info(url_vul, resp_vul)  # 探测敏感信息
                if path != '/gateway/actuator/env':
                    continue
            except:
                return False
    ...
    async def send_request(self, url):
        ...
        try:
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                async with sem:
                    async with session.get(url, timeout=10, headers=self.headers) as req:
    ...
    def extract_link(self, parse_url, link):
        ...
        try:
            ...
            if full_url not in self.apis and file_extend != 'html' and file_extend != 'js':
                #status = self.send_request_api(full_url)    #获取api接口响应状态码
